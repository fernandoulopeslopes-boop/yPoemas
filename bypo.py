...
# - BYPO NÃO importa nem altera basico.py.
# - Conteúdo real: mini, yPoemas, eureka, off-mach e ABOUT.
# - Arquitetura BYPO: sidebar 320 px + palco horizontal; sidebar recolhível.
#
# HISTÓRICO RECENTE DA LINHA BYPO
# - 019 REAL: conteúdo funcional real incorporado ao gabarito BYPO.
# - 020 CAE_PROFUNDO: removido CSS estrutural legado e regressões do basico.
# - 021 RETRATO_FONTE_ESTILO: retrato ligado à fonte/estilo da sidebar.
# - 022 RETRATO_FONTE_REAL: cadeia de fallback de fonte reorganizada.
# - 023 RETRATO_FONTE_FISICA_ACOES_CENTRO: vínculo físico de fonte + ações centrais.
# - 024 TRIO_IGUAL_RETRATO_FONTE_SIDEBAR: trio livro/nav/tema 1:1:1 e estado de fonte corrigido.
# - 025 FONTES_EXTERNAS_SEM_OPENDYSLEXIC_HARDCODE: removida autoridade interna OpenDyslexic.
# - 026 SEM_LISTAS_HARDCODE: livros/off/fontes deixam de ter catálogos paralelos no código.
# - 027 TIPOGRAFIA_RERUN: fonte/estilo/corpo da sidebar aplicam rerun imediato no palco.
...
# -> 1 Mini -> 2 yPoemas -> 3 Eureka -> 4 Off-Machina
...
def load_livros_list():
    """Lista autoral de livros = arquivos reais base/rol_*.txt."""
    base_dir = _project_path("base")
    livros = []
    try:
        for nome in os.listdir(base_dir):
            nome_cf = nome.casefold()
            if not (nome_cf.startswith("rol_") and nome_cf.endswith(".txt")):
                continue
            livro = nome[4:-4].strip()
            if livro:
                livros.append(livro)
    except OSError:
        return []

    livros = sorted(dict.fromkeys(livros), key=str.casefold)
    if "todos os temas" in livros:
        livros.remove("todos os temas")
        livros.insert(0, "todos os temas")
    return livros


def load_off_livros_list():
    """Lista Off-Machina = arquivos .Pip reais da pasta off_machina."""
    off_dir = _project_path("off_machina")
    livros = []
    try:
        for nome in os.listdir(off_dir):
            stem, ext = os.path.splitext(nome)
            if ext.casefold() == ".pip" and stem.strip():
                livros.append(stem.strip())
    except OSError:
        return []
    return sorted(dict.fromkeys(livros), key=str.casefold)
...
def apply_styles():
    ...
    st.markdown(
        ...
        /* BYPO: sidebar nativa do Streamlit não participa desta arquitetura. */
        ...
        /* BYPO: geometria do conjunto pertence aos containers Streamlit. */
...
def apply_bypo_styles():
    """Acabamento do BYPO; a geometria principal continua nativa do Streamlit."""
    st.markdown(
        """
        <style>
        ...
        .st-key-bypo_sidebar_toggle div[data-testid="stButton"] button {
...
def init_session_state():
    ...
    defaults = {
        ...
        "sidebar_house_open": True,
        "bypo_sidebar_panel": "image",
...
def _sidebar_house_toggle():
    st.session_state["sidebar_house_open"] = not bool(
        st.session_state.get("sidebar_house_open", True)
    )


def _sidebar_house_open():
    return bool(st.session_state.get("sidebar_house_open", True))


def _sidebar_space(rem=1.0):
    _sidebar_host().markdown(
        f"<div class='machina-sidebar-space' style='height:{float(rem):.2f}rem'></div>",
        unsafe_allow_html=True,
    )
    ...
    for label, filename in _load_fontes_ypoemas():
        ...
        try:
            with open(caminho, "rb") as fonte:
                payload = base64.b64encode(fonte.read()).decode("ascii")
        except OSError:
            continue
...
def pick_livro_palco():
    ...
    books_list = load_livros_list()
    if not books_list:
        st.warning("nenhum arquivo base/rol_*.txt encontrado")
        return
    current = _current_book()
    ...
    st.selectbox(
        f"livro: {books_list.index(current) + 1} / {len(books_list)}",
        books_list,
...
def pick_tema_palco():
    ...
    st.selectbox(
        f"tema: {fix_take(st.session_state.get('take', 0), temas) + 1} / {len(temas)}",
        options,
...
def _sidebar_tipografia_rerun():
    """Sincroniza fonte/estilo/corpo e refaz a tela imediatamente."""
    fonte = st.session_state.get("sidebar_font_select")
    estilo = st.session_state.get("sidebar_style_select")
    corpo = st.session_state.get("sidebar_size_select")

    if fonte:
        st.session_state.fonte_palco = fonte
    if estilo in ESTILOS_MACHINA:
        st.session_state.estilo_palco = estilo
    try:
        if corpo is not None:
            st.session_state.corpo_palco = int(corpo)
    except (TypeError, ValueError):
        pass

    # Retratos já materializados pertencem à tipografia anterior.
    limpar_retratos()
...
def pick_fonte_palco():
    ...
    choice = _sidebar_host().selectbox(
        translate("fonte"),
        labels,
        index=labels.index(current_font),
        key="sidebar_font_select",
        on_change=_sidebar_tipografia_rerun,
    )
    col_estilo, col_corpo = _sidebar_host().columns([1.65, 1.15])
    with col_estilo:
        estilo = st.selectbox(
            translate("estilo"),
            ESTILOS_MACHINA,
...
def _bypo_toggle_links():
    atual = st.session_state.get("bypo_sidebar_panel", "image")
...
def _bypo_context_image_path(chosen_id):
    ...
    if chosen_id in {"1", "2", "3"}:
        return st.session_state.get("save_image_tema", "")
    if chosen_id == "4":
        return st.session_state.get("off_machina_images_pasta", "")
    if chosen_id == "5":
        return st.session_state.get("about_image", "") or _set_about_image_next()
    return ""


def _bypo_render_context_image(chosen_id):
    image_path = _bypo_context_image_path(chosen_id)
...
def limpar_retratos():
    ...
    for prefixo in ("mini", "ypo", "eureka", "off"):
...
def _aplicar_selo_origem(canvas, size, respiro=RETRATO_SELO_RESPIRO, family=None, estilo=None, arquivo_fonte=None):
    """Assina o Retrato com yP + endereço de origem no canto inferior direito."""
    logo = _retrato_logo_yp(size)
    if logo is None:
        return canvas

    url_font = _retrato_font(max(16, int(round(size * 0.72))), family=family, estilo=estilo, filename=arquivo_fonte)
    url = RETRATO_ORIGEM_URL
    text_w, text_h = _retrato_text_size(url, url_font, estilo=estilo)
    ...
    text_y = pos_y + max(0, (size - text_h) // 2)
...
def _retrato_font(size, bold=False, family=None, estilo=None, filename=None):
    ...
    def _style_variants(filename, bold=False, italic=False):
        if not filename:
            return []
        ...
        if bold and italic:
            targets = [
                ...
                "ItalicBold", "Italic-Bold", "Italic_Bold",
                "BoldOblique", "Bold-Oblique", "Bold_Oblique",
                "SemiBoldItalic", "SemiBold-Italic", "SemiBold_Italic",
            ]
        elif bold:
            targets = ["Bold", "SemiBold", "Semibold", "Black"]
        elif italic:
            targets = ["Italic", "Oblique"]

        # Primeiro tenta a variante PEDIDA; o regular/base vem por último.
        if "regular" in lower:
            for target in targets:
                add(re.sub("regular", target, base, flags=re.IGNORECASE))
        if "normal" in lower:
            for target in targets:
                add(re.sub("normal", target, base, flags=re.IGNORECASE))
        if targets:
            for target in targets:
                add(base + "-" + target)
                add(base + target)
    ...
    # 2) A lista externa é a autoridade. Não reinterpretar a família escolhida.
    # Se o arquivo publicado não puder ser carregado, só então usa fonte genérica do SO.

    # 3) Último recurso técnico: fonte genérica do SO. Nunca outra fonte autoral.
    ...
    for candidate in sys_candidates:
        loaded = _try_font(candidate)
        if loaded is not None:
            return loaded
    return ImageFont.load_default()


def _retrato_assinatura_contexto(contexto_base):
    """Assina o retrato também pela fonte e estilo escolhidos no palco."""
    base = tuple(contexto_base or ())
    return base + (
        str(_fonte_palco_leitor() or ""),
        str(_estilo_palco_leitor() or "normal"),
    )

def _retrato_text_image(texto, font, estilo="normal", fill=(0, 0, 0, 255)):
    """Renderiza texto com estilo garantido, mesmo sem arquivo físico bold/italic separado."""
    texto = str(texto or "")
    estilo_cf = str(estilo or "normal").strip().casefold()
    want_bold = "bold" in estilo_cf
    want_italic = "itálico" in estilo_cf or "italic" in estilo_cf
    stroke = 1 if want_bold else 0

    probe = Image.new("RGBA", (1, 1), (0, 0, 0, 0))
    draw = ImageDraw.Draw(probe)
    bbox = draw.textbbox((0, 0), texto or " ", font=font, stroke_width=stroke)
    ...
    if want_italic:
        shear = 0.22
        extra = max(1, int(round(height * shear)))
...
def _retrato_text_size(texto, font, estilo="normal"):
    img = _retrato_text_image(texto, font, estilo=estilo)
    return img.size


def _retrato_draw_text(canvas, xy, texto, font, estilo="normal", fill=(0, 0, 0, 255)):
    img = _retrato_text_image(texto, font, estilo=estilo, fill=fill)
...
def criar_retrato_png(ypoema_html, image_path, tema, selo_size=24, fonte_retrato=None, estilo_retrato=None, arquivo_fonte_retrato=None):
    ...
    corpo_png = 42
    body_font = _retrato_font(
        corpo_png, family=fonte_retrato, estilo=estilo_retrato, filename=arquivo_fonte_retrato
    )
    title_font = _retrato_font(
        max(corpo_png + 4, int(round(corpo_png * 1.18))),
        family=fonte_retrato, estilo=estilo_retrato, filename=arquivo_fonte_retrato
    )
    ...
    _ag_w, ag_h = _retrato_text_size("Ag", body_font, estilo=estilo_retrato)
    ...
    footer_text_w, _footer_text_h = _retrato_text_size(RETRATO_ORIGEM_URL, footer_font, estilo=estilo_retrato)
    ...
    for linha in linhas:
        ...
        if linha:
            _retrato_draw_text(canvas, (text_x, pos_y), linha, body_font, estilo=estilo_retrato)
        pos_y += line_h

    _aplicar_selo_origem(canvas, selo_real, RETRATO_SELO_RESPIRO, family=fonte_retrato, estilo=estilo_retrato, arquivo_fonte=arquivo_fonte_retrato)
    output = BytesIO()
...
def _render_eureka_registro(texto_html, texto_copia, tema, imagem, key_prefix, sem_hints=False):
    ...
    if contexto_retrato and tuple(contexto_retrato) != _retrato_assinatura_contexto(contexto):
        limpar_retrato(prefixo)
...
def load_about_catalog():
    """Retorna o catálogo ABOUT conforme a variante em execução.
    ...
    BYPO_CFG:
    - autoridade de curadoria: arquivos reais ./md_files/*.md
    - nenhum catálogo intermediário; lista todos os .md em ordem alfabética.
    """
    if APP_VARIANT == "bypo_cfg":
        ...
        md_files = sorted(
            (name for name in os.listdir(md_dir)
             if os.path.isfile(os.path.join(md_dir, name)) and name.casefold().endswith(".md")),
            key=str.casefold,
        )
        return [(name, name) for name in md_files]
    ...
    if lnew or st.session_state.auto:
        ...
        with mini_expander:
            ...
            if st.session_state.auto == False:
                ...
                if st.session_state.get("mini_contexto_retrato") and tuple(st.session_state.get("mini_contexto_retrato")) != _retrato_assinatura_contexto(mini_contexto):
...
def page_ypoemas():
    ...
    try:
        ...
        col_livros, col_nav, col_temas = st.columns([1, 1, 1])
    ...
    with col_nav:
        ...
        if machina_nav_needs_spacer:
            st.markdown(
                "<div style='height:1.45rem; min-height:1.45rem;'></div>",
                unsafe_allow_html=True,
            )
    ...
    if lnew:
        ...
        with ypoemas_expander:
            ...
            if st.session_state.get("ypo_contexto_retrato") and tuple(st.session_state.get("ypo_contexto_retrato")) != _retrato_assinatura_contexto(ypo_contexto):
...
def page_off_machina():  # available off_machina_books
    ...
    if off_retrato_contexto_salvo and tuple(off_retrato_contexto_salvo) != _retrato_assinatura_contexto(off_retrato_contexto_atual):
        limpar_retrato_off()
...
# =============================================================================
def page_about():
    """ABOUT público curado; no BYPO_CFG, bancada completa de /md_files/*.md."""
    catalog = load_about_catalog()
    if not catalog:
        if APP_VARIANT == "bypo_cfg":
            st.warning(translate("md_files sem arquivos .md ou não encontrado"))
        else:
...
# =============================================================================
def _bypo_render_page_menu(nav_items, page_ids, sidebar_open):
    def render_pages():
        page_cols = st.columns(len(nav_items), gap="small")
        for (display_label, target_label, _weight), page_col in zip(nav_items, page_cols):
            with page_col:
                ...
                if st.button(
                    display_label,
                    key=f"bypo_page_btn_{display_label}",
                    use_container_width=True,
    ...
    with st.container(key="bypo_page_menu", border=False):
        if sidebar_open:
            render_pages()
        else:
            toggle_col, pages_col = st.columns([0.34, 5.0], gap="small")
            with toggle_col:
                with st.container(key="bypo_sidebar_toggle", border=False):
                    st.button(
                        "☰",
                        key="bypo_menu_open",
...
def _bypo_render_real_page(chosen_id):
    if chosen_id == "1":
        page_mini()
        return f"🍃  {st.session_state.lang} - {st.session_state.tema} ( {st.session_state.mini + 1} / {len(load_temas('todos os temas'))} )"
    if chosen_id == "2":
        page_ypoemas()
        current_book = _current_book()
    ...
    if chosen_id == "3":
        page_eureka()
        return palco_status("eureka")
    if chosen_id == "4":
        page_off_machina()
        return palco_status("off-machina")
    if chosen_id == "5":
        page_about()
...
def start_machina(app_variant="bypo"):
    ...
    if st.session_state.visy:
        ...
        if temas:
            st.session_state.take = random.randrange(0, len(temas))
    ...
    if sidebar_open:
        with st.container(horizontal=True, gap="small", vertical_alignment="top"):
            with st.container(
                key="bypo_sidebar_frame",
                width=BYPO_SIDEBAR_WIDTH_PX,
                border=True,
            ):
        ...
        with st.container(key="bypo_palco_full", width="stretch", border=True):
            _bypo_render_page_menu(nav_items, page_ids, sidebar_open=False)
            chosen_label = st.session_state["pick_pagina"]
            chosen_id = str(st.session_state.get("pagina", page_ids.get(chosen_label, "2")))
            with st.container(
