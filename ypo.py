...
# --- 1. A CARA DA MÁQUINA (CSS RADICAL) ---
...
# --- 2. O ESTADO É A MEMÓRIA DA ORDEM ---
...
with st.sidebar:
    # Idiomas: O clique aqui muda a 'alma' e o Streamlit já recarrega sozinho
    ...
    if c6.button("⚒️"): st.session_state.lang = 'es' # Exemplo de Poly
...
# Botões de Navegação: Clique = Nova Posição
...
# --- 5. EXECUÇÃO DA POESIA (O RESULTADO DA ORDEM) ---
tema_atual = temas[st.session_state.take]
# gera_poema já recebe a ordem de idioma do session_state.lang
...
if st.session_state.draw and os.path.exists(img_path):
    ...
    html_palco = f'''<div class="container">
                        ...
                        <p class="logo-text">{texto_corpo}</p>
                     </div>'''
else:
    html_palco = f'<div class="container"><p class="logo-text">{texto_corpo}</p></div>'
