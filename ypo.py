...
def translate(input_text):
    ...
    try:
        ...
        return translate("Arquivo muito grande para ser traduzido.", + st.session_state.lang)
...
def page_ypoemas():
    ...
    if not st.session_state.draw:
        ...
        sobrios = "↓  " + translate("lista de Temas" + st.session_state.lang)
        ...
        if len(seed_list) == 0:
            st.warning(
                translate(
                    ...
                    + find_what
                    + ' " foi encontrada...' + st.session_state.lang
                )
            )
        elif len(seed_list) >= 1:
            seed_list.sort()
            if len(seed_list) == 1:
                info_find = translate('ocorrência de "' + st.session_state.lang)
            else:
                info_find = translate('ocorrências de "' + st.session_state.lang)

            info_find += find_what
            if len(soma_tema) > 1:
                info_find += translate('" em ' + str(len(soma_tema) + st.session_state.lang) + " temas")
