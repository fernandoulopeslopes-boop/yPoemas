A Máquina (A) - O que é  
___  
Um aplicativo para gerar poemas ou textos aleatoriamente.  

**Visão geral** O trabalho é dividido em temas. Atualmente  
são 144.  
Com um total aproximado de 40.600 palavras, são criados  
Bancos de Ítimos para cada tema.  

**Como funciona** O texto é criado em três dimensões-eixos  
onde a leitura convencional multiplica-se a cada mudança no  
eixo Z.  
A máquina distribui o texto em camadas:  
frases no eixo X, linhas no eixo Y e palavras extraídas do  
imaginário eixo Z.  

> Escolhido um tema, o aplicativo seleciona e embaralha  
aleatoriamente  
> as palavras do **banco** ligado ao mesmo e reorganiza-as  
> em um novo texto, único, original e diferente do anterior.  
> Cada texto gerado é um novo **yPoema**  

> Em uma visão espacial, o texto é criado em três  
dimensões-eixos onde a leitura convencional multiplica-se  
à cada mudança no **eixo Z**.  

> A **máquina** distribui o texto em camadas.  
As frases são escritas no eixo X, as linhas no eixo Y,  
e as palavras de cada frase são extraídas do **imaginário  
eixo Z**.  
___  

---  
Copyright © 1983-Hoje Nando Lopes - **yPoemas @ a Machina de fazer Poesia**
