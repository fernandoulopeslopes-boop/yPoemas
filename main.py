...
def buscar_doc(nome_pagina):
    ...
    # Mapeamento para garantir que yPoemas.md seja encontrado
    mapa = {
    ...
    arquivos_no_disco = os.listdir(pasta)
    
    for f in arquivos_no_disco:
...
with c1:
    ...
    if os.path.exists("base"):
        ...
        if lista_livros:
            ...
            idx_s = st.session_state.idx_tema % len(st.session_state.temas_atuais)
...
with c2:
    ...
    # Help: toast efêmero para não travar a renderização
    ...
    if st.session_state.page == "demo":
        ...
        if conteudo:
            ...
            st.warning(f"O documento '{st.session_state.page}' não foi localizado.")import streamlit as st
import os
import random
