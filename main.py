...
# --- 1. CONFIGURAÇÃO DE HARDWARE (RESTAURO 10/ABRIL) ---
st.set_page_config(
...
# --- 2. CSS: BLINDAGEM DA SIDEBAR E SIMETRIA TOTAL ---
st.markdown("""
    <style>
    /* REMOVER LIXO NATIVO */
    [data-testid="stHeader"] { display: none !important; }
    
    /* SIDEBAR BLINDADA (320px) - GARANTE QUE ELA APAREÇA */
    ...
    /* BOTÕES DE NAVEGAÇÃO: LARGURA IDÊNTICA (container_width) */
    .stButton > button {
    ...
    /* RÉGUA: OS 5 BOTÕES QUADRADOS DA IMAGEM */
    ...
    .info-box {
        ...
        color: #1a1a1a; background: #fff; padding: 15px; border-left: 5px solid #000;
    }

    /* CENTRALIZAÇÃO DO PALCO */
...
# --- 5. A RÉGUA DE COMANDO (SIMETRIA DA LAST_SCREENSHOT) ---
...
# Linha 2: Seletores (Varredura de arquivos na pasta \base)
...
# IDIOMA (Régua)
with row_sel[1]:
    st.selectbox("Sel_Lang", idiomas[:3], key="s_lang", label_visibility="collapsed")

# GRUPO (Leitura automática de \base\rol_*.txt)
with row_sel[2]:
    try:
        # Busca real: ensaios, poemas, sociais, etc.
        grupos_base = [f.replace("rol_", "").replace(".txt", "") for f in os.listdir("base") if f.startswith("rol_")]
        grupo_selecionado = st.selectbox("Sel_Grupo", sorted(grupos_base), key="s_group", label_visibility="collapsed")
    except:
        grupo_selecionado = st.selectbox("Sel_Grupo", ["todos os temas"], key="s_group_err", label_visibility="collapsed")

# TEMA (Leitura do conteúdo do arquivo selecionado)
with row_sel[3]:
    try:
        with open(f"base/rol_{grupo_selecionado}.txt", "r", encoding="utf-8") as f:
            temas_lista = [line.strip() for line in f.readlines() if line.strip()]
        st.selectbox("Sel_Tema", temas_lista, key="s_tema", label_visibility="collapsed")
    except:
        st.selectbox("Sel_Tema", ["Amaré"], key="s_tema_err", label_visibility="collapsed")
