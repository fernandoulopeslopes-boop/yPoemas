...
def main():
    ...
    if 'book_em_foco' not in st.session_state: 
        st.session_state.book_em_foco = "poemas"
    if 'tema_idx_por_book' not in st.session_state: 
        st.session_state.tema_idx_por_book = {b: 0 for b in MAPA_BOOKS}
    if 'seed_eureka' not in st.session_state: 
        st.session_state.seed_eureka = 0
    if 'help_ativo' not in st.session_state: 
        st.session_state.help_ativo = False

    aba_atual = PAGINAS_APP[st.session_state.current_tab_idx]
    
    if aba_atual == "mini": 
        book_em_foco = "temas mini"
    elif aba_atual == "eureka": 
        book_em_foco = "livro vivo"
    else: 
    ...
    with st.sidebar:
        ...
        novo_book = st.selectbox("Livro", lista_livros, index=idx_livro_foco, label_visibility="collapsed", key="sb_book_select")
