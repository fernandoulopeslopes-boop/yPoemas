...
def normalizar_e_traduzir(texto, idioma):
    if not texto: return ""
    ...
    if "Português" not in idioma:
        try:
            ...
            texto_final = GoogleTranslator(source='auto', target=target).translate(texto)
        except: pass

    # Simulação do "Save/Load Typo" para normalizar quebras
    # Substitui quebras duplas acidentais por simples
...
def aplicar_estetica_machina():
    st.markdown("""
        <style>
            ...
            /* Botões Circulares com Espaçamento (Gap) */
            div.stButton > button {
                ...
                margin: 0 5px !important; /* Respiro entre os botões */
            }
        </style>
...
def main():
    ...
    # --- CONTROLES DO PALCO (6 COLUNAS PARA INCLUIR O "?") ---
    ...
    if c5.button("?"): 
        st.toast("Ajuda: Use os botões para navegar nas variações da Machina.")
