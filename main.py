...
# --- DICIONÁRIO DE CURADORIA (EXATIDÃO VISUAL) ---
DIC_TEMAS_GRUPOS = {
...
def buscar_arte_curada(tema):
    ...
    if os.path.exists(pasta_grupo):
        ...
        if arquivos: return os.path.join(pasta_grupo, random.choice(arquivos))
    ...
    if os.path.exists(pasta_maquina):
        ...
        if arquivos_m: return os.path.join(pasta_maquina, random.choice(arquivos_m))
    return None
...
LISTA_IDIOMAS = [
    "PT - Português", "EN - English", "ES - Español", "IT - Italiano", "FR - French", "DE - German",
    "AF - Afrikaans", "SQ - Albanian", "CA - Catalan", "HR - Croatian", "CS - Czech", "DA - Danish",
    "NL - Dutch", "ET - Estonian", "FI - Finnish", "HU - Hungarian", "IS - Icelandic", "ID - Indonesian"
]
