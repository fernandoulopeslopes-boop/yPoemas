    ...
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        ...
        if seed:
            ...
            for l in linhas:
                ...
                if len(p) >= 4 and p[3].strip() == target_id:
                    # Coleta a 7ª coluna, mas se houver pipes internos (variantes), sorteia uma
                    res = p[7].strip()
                    if "|" in res:
                        vars = [v.strip() for v in res.split("|") if v.strip()]
        ...
        for l in linhas:
            ...
            if len(p) >= 8:
                txt = p[7].strip()
                if txt and txt != "?":
                    # Trata variantes dentro da própria coluna 7
                    if "|" in txt:
                        pool.extend([v.strip() for v in txt.split("|") if v.strip()])
                    else:
                        pool.append(txt)
...
def page_ypoemas():
    ...
    temas = get_rol(livro)
    if not temas: 
        st.warning("Selecione um acervo válido em 'books'.")
        return
    
    if f'idx_{livro}' not in st.session_state: st.session_state[f'idx_{livro}'] = 0
    
    c = st.columns([1,1,2,1,1])
    if c[0].button("➕", key="y1"): st.rerun()
    if c[1].button("◀", key="y2"): st.session_state[f'idx_{livro}'] -= 1
    if c[2].button("✻", key="y3"): st.session_state[f'idx_{livro}'] = random.randrange(len(temas))
    if c[3].button("▶", key="y4"): st.session_state[f'idx_{livro}'] += 1
    
    st.session_state[f'idx_{livro}'] %= len(temas)
    t = temas[st.session_state[f'idx_{livro}']]
...
def page_off_machina():
    ...
    if not os.path.exists(PATH_OFF): return
...
def page_books():
    ...
    st.session_state.current_book = st.radio("Acervo Ativo:", rois, index=rois.index(st.session_state.current_book) if st.session_state.current_book in rois else 0)
...
def main():
    ...
    # Tab Bar fixa para garantir acesso às páginas
