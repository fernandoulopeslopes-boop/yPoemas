...
# 2. INJEÇÃO DE CSS (Sem aspas triplas para estabilidade no interpretador)
...
# 3. ESTADO DA SESSÃO (Inicialização segura)
...
def ler_arquivo_help(caminho):
    if os.path.exists(caminho):
        try:
            with open(caminho, "r", encoding="utf-8") as f:
                return f.read()
        except Exception:
            return "Erro ao ler arquivo de ajuda."
    return "Help indisponível."
...
def render_sidebar():
    with st.sidebar:
        ...
        # Renderização da Imagem da Página
        ...
        # Controles de Idioma e Flags de Estado
...
def render_palco():
    ...
    if p == "mini":
        ...
        txt_help = ler_arquivo_help("md_files/help_ypoemas.md")
        c4.button("?", key="m_h", help=txt_help)
    ...
    # 2. YPOEMAS
        ...
        txt_help = ler_arquivo_help("md_files/HELP_YPOEMAS.MD")
        c4.button("?", key="y_h", help=txt_help)
        st.info("under construction...")
    ...
    # 3. EUREKA
        ...
        txt_help = ler_arquivo_help("md_files/HELP_EUREKA.MD")
        c3.button("?", key="e_h", help=txt_help)
        st.info("under construction...")
    ...
    # 4. LIVROS
        ...
        if files:
            ...
            txt_help = ler_arquivo_help("md_files/HELP_OFF-MACHINA.MD")
            c5.button("?", key="l_h", help=txt_help)
    ...
    # 5. POLY
        ...
        if files:
            ...
            txt_help = ler_arquivo_help("md_files/HELP_POLY.MD")
            c2.button("?", key="p_h", help=txt_help)
    ...
    # 6. SOBRE
        ...
        abouts = glob.glob("md_files/ABOUT_*.MD") + glob.glob("md_files/ABOUT_*.md")
        if abouts:
            # Elimina duplicatas mantendo consistência de caminhos únicos
            abouts = list(set(abouts))
