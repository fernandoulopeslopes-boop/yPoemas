...
st.markdown("""
    ...
    /* BOTÕES CIRCULARES DO PALCO (Console de 6 Botões) */
...
# --- 2. SIDEBAR: COCKPIT DE CONTROLE ---
with st.sidebar:
    st.markdown("<div style='margin-top: 20px;'></div>", unsafe_allow_html=True)
    
    # A. IDIOMAS: Lista Completa (Elite + Western)
    ...
    <div class='info-box'>
        ...
        Processando matriz rítmica. Sincronia de tradução ativa para os idiomas selecionados.
    </div>
    """, unsafe_allow_html=True)
    
    # D. ARTE DA PÁGINA (Substituindo use_container_width por width='stretch')
...
with col_barra:
    ...
    with c_btns:
        ...
        # Console de 6 botões conforme o Retrato
        n1, n2, n3, n4, n5, n6 = st.columns(6)
        n1.button("＋", key="p_add")   # Novo
        n2.button("＜", key="p_prev")  # Voltar
        n3.button("＊", key="p_star")  # Aleatório
        n4.button("＞", key="p_next")  # Avançar
        n5.button("☐", key="p_auto")  # Automático
        n6.button("？", key="p_help")  # Info
        st.markdown("</div>", unsafe_allow_html=True)

    with c_lista:
        # Carregamento dinâmico dos temas (.ypo) da pasta data/
        try:
            ...
            st.selectbox("Temas", ["Proust", "Metafísica"], key="p_temas_error")
...
<div style='text-align: center; color: #1a1a1a; font-family: Georgia; margin-top: 40px; font-style: italic;'>
    O Palco está centralizado. Aguardando a entrada dos versos da Máquina.
</div>
""", unsafe_allow_html=True)
