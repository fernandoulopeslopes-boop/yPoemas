...
def show_ypoemas():
    ...
    if n1.button(get_h(0)): st.session_state.take -= 1
    if n2.button(get_h(1)): st.session_state.take = random.randint(0, 999)
    if n3.button(get_h(2)): st.session_state.take += 1
    with n4:
        with st.expander(get_h(3)):
            st.write("Instruções originais.")
