...
# CRONOLOGIA ATIVA: X=30 (CONSOLIDAÇÃO: Topo Zero Real + Curadoria Latina + Escopo Global)

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
BASE_PATH = os.path.join(BASE_DIR, "base")

# Escopo Global: Curadoria restrita e precisa
DICI_LANG = {
    'Português': 'pt', 
    'English': 'en', 
    'Español': 'es', 
...
def aplicar_estetica_v30():
    st.markdown("""
        <style>
            ...
            /* Eixo Central Controlado */
            .central-axis { width: 100%; max-width: 450px; margin: 0 auto; }
            ...
            /* Width Reduzido (-40%) e Centralizado */
...
def main():
    ...
    with col_c:
        ...
        # Linha 2: Temas (Posição e Width rigorosos)
        ...
        st.selectbox("T", lista_temas, index=idx_tema, key=f"v30_{idx_tema}", 
                     on_change=lambda: st.session_state.memoria_temas.update({book_foco: lista_temas.index(st.session_state[f"v30_{idx_tema}"])}),
                     label_visibility="collapsed")

    # Linha 3: Navegação de Páginas (Habilitada)
    ...
    if aba_atual in ["demo", "ypoemas"]:
        ...
        st.markdown("### a Máquina de Fazer Poesia\nVersão 2026.04 — Eixo Centralizado e Topo Zero.")
    else:
        st.info(f"Página {aba_atual.upper()} ativa. Aguardando integração de conteúdo.")
