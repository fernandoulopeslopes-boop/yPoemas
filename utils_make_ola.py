...
def _utils_atualizar_rodape_ypo_um_tema(tema, path):
    ...
    novas = _utils_linhas_rodape_ypo(path)
    chaves = {
        "verbetes no texto",
        "total de ítimos",
        "total de itimos",
    ...
    if novo_texto != texto_atual:
        _utils_write_text(path, novo_texto)
...
def _make_ola_texto_utf8(raw_bytes):
    """Lê texto UTF-8 estrito para uma única análise OLA."""
    try:
        texto = bytes(raw_bytes or b"").decode("utf-8-sig")
    except UnicodeDecodeError as exc:
        raise ValueError("O arquivo não é texto UTF-8. Nenhuma análise foi solicitada.") from exc
...
def render_make_ola_tool():
    """Envia um texto UTF-8 uma única vez para análise sintática da OLA."""
    st.markdown("### make_ola")
    st.caption(
        "Escolha um arquivo textual UTF-8. O nome do arquivo identifica o tema. "
        "Cada clique faz uma única chamada OLA: análise sintática."
    )
    ...
    tema = os.path.splitext(os.path.basename(str(uploaded.name or "texto")))[0].strip()
    tema = tema or "texto"
    st.caption("tema da análise: " + tema)
    ...
    if st.button("make_ola", width="stretch"):
        if _gerar_analise_ola_real is None:
            st.error(
                "OLA não conectada. O arquivo ponte_ola_openai.py "
                "não foi encontrado ou não pôde ser importado."
            )
            return

        try:
            texto_fonte = _make_ola_texto_utf8(uploaded.getvalue())
            with st.spinner("OLA — análise sintática..."):
                # Uma única chamada. Tipo fixo: Sintática.
                analise = _gerar_analise_ola_real("Sintática", tema, texto_fonte)

            analise = str(analise or "").strip()
            if not analise:
                raise RuntimeError("A OLA não devolveu texto.")

            st.session_state["make_ola_result"] = analise
            st.session_state["make_ola_result_name"] = f"OLA_{tema}.txt"
            st.success("make_ola concluído: uma análise sintática.")
        except Exception as exc:
            st.error(f"make_ola falhou: {exc}")

    resultado = str(st.session_state.get("make_ola_result", "") or "")
    if resultado:
        nome_saida = st.session_state.get("make_ola_result_name", f"OLA_{tema}.txt")
        st.text_area(
            "análise sintática OLA",
            value=resultado,
            height=420,
            key="make_ola_result_textarea",
        )
        st.download_button(
            "baixar " + nome_saida,
            data=resultado + "\n",
...
make_ola
  Lê um arquivo textual UTF-8 e faz uma única chamada à OLA para análise sintática.
  Usa o nome do arquivo como tema e permite copiar ou baixar o resultado.
