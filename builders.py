...
def _temas_ativos():
    ...
    with open(ativos_path, encoding="utf-8") as file:
        for raw in file:
            ...
            tema = linha.partition(" : ")[0].strip()
            if tema:
                temas.append((tema, _resolve_ypo_path(tema)))
    return temas


def _linhas_ypo(path):
...
def _palavras_de_itimo(itimo, minimo=1):
...
def build_lexico():
    ...
    for tema, path in _temas_ativos():
        if not os.path.exists(path):
            continue
        for campos in _linhas_ypo(path):
            if len(campos) < 8:
                continue
            fonte = campos[3]
            for itimo in _payload_itimos(campos):
                for palavra in _palavras_de_itimo(itimo, minimo=3):
                    chave = palavra + " : " + fonte
    ...
    _write_text(os.path.join(base_dir, "lexico_pt.txt"), texto_lexico)
    _write_text(os.path.join(base_dir, "verbetes.txt"), texto_verbetes)
    return (
...
def _calcular_variacoes_tema(path):
    fontes_list = []
    corrige_qtd = 1
    qtd_itimos_list = []
    for campos in _linhas_ypo(path):
        if len(campos) >= 8:
            nova_fonte = campos[3]
            total_itimos = len(_payload_itimos(campos))
...
def build_indexy():
    ...
    for tema, path in temas:
        try:
            qtd_variatio = _calcular_variacoes_tema(path)
            acm_variatio += qtd_variatio
            index_list.append(f"{tema} : {qtd_variatio:,} ({_potencia_nome(qtd_variatio)})")
    ...
    linhas.extend([
        ...
        "",
        f"Total de variações: {acm_variatio:,} ({_potencia_nome(acm_variatio)})".replace(",", "."),
        "",
    ])
    _write_text(about_index, "\n".join(linhas))
